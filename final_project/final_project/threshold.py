...
if __name__ == "__main__":
    import cv2
    # 讀取圖像並轉為灰階
    gray = cv2.imread('./photo/test.jpg', cv2.IMREAD_GRAYSCALE)
    
    # 自製二值化函數
    binary = threshold(gray, 127, 255)
    
    # 使用 OpenCV 的方法對比
    _, binary_cv = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
    
    # 顯示結果
    cv2.imshow("Custom Binary", binary)
    cv2.imshow("OpenCV Binary", binary_cv)
    cv2.waitKey(0)
